...
## Why AWS DynamoDB for NoSQL?

AWS DynamoDB is the superior choice for this application's NoSQL database over Azure Cosmos DB for three reasons:

**Cost:** DynamoDB's on-demand (`PAY_PER_REQUEST`) pricing means you pay only per read/write request with no minimum capacity commitment — ideal for variable or unpredictable workloads. Azure Cosmos DB requires a minimum provisioned throughput (400 RU/s ≈ $23/month), making it more expensive at low usage levels.

**Availability:** DynamoDB offers a 99.999% SLA for global tables with multi-region, multi-active replication built in. Cosmos DB offers comparable availability but at a significantly higher cost tier.

**Technology:** DynamoDB is fully serverless with automatic scaling and zero administration overhead. Its native integration with other AWS services already in this deployment (ECS, S3) simplifies IAM policies and reduces operational complexity.
