    ...
    "environment": [
      {
        ...
        "value": "udacity-ali-azure-sql"
      },
      {
        "name": "AZURE_DOTNET_APP",
        "value": "udacity-ali-azure-dotnet-app"
      }
    ],
    "portMappings": [
      {
...
# S3 Bucket — referenced by the Azure container app as AWS_S3_BUCKET
resource "aws_s3_bucket" "udacity" {
  bucket = "udacity-ali-aws-s3-bucket"
  tags   = local.tags
}

# DynamoDB Table — referenced by the Azure container app as AWS_DYNAMO_INSTANCE
resource "aws_dynamodb_table" "udacity" {
  name         = "udacity-ali-aws-dynamodb"
